...
2. 입력 받을 때 Map에 추가
3. 입력을 받으면서 value를 비교해서 최댓값인 index의 key 구하기
4. value 최댓값이 같은 경우는 -1을 대입

## 회고
답을 구하기 위한 핵심 로직을 메서드로 구현해서 풀었는데,   
처음부터 입력값을 돌리면서 구하는 방법이 아직 미숙한 것 같다.

다음 문제부터 반복문 안에서 입력값을 받으면서 문제가 풀릴 수 있는지 먼저 접근해 봐야겠다.
